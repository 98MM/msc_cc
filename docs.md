...
- **class_relation**: _str_, either _'linear'_, _'nonlinear'_, _'cluster'_, or _'feature-like'_, default='linear':
  Sets relationship type between class label and sample, by calculating a decision boundary with linear or nonlinear combinations of features in X, by clustering the samples in X, or by generating a target vector based on a given feature.
- **balance**: _boolean_, default=False:
  Whether to naievly balance clusters generated by KMeans clustering.
- **random_state**: _int_, default=42:
  Random state seed for KMeans clustering.
- **feature_vector**: _list_ or _numpy.ndarray_:
  Feature vector to base labels on.
